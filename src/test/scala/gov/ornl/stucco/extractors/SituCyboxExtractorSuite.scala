...
class SituCyboxExtractorSuite extends FunSuite {
  ...
  test("parse one complete record") {
    ...
    //assert(entries ~> "vertices" ~> 0 ~> "duration" === Some(S("100")))
    //assert(entries ~> "vertices" ~> 0 ~> "srcAppBytes" === Some(S("101")))
    //assert(entries ~> "vertices" ~> 0 ~> "dstAppBytes" === Some(S("102")))
    //assert(entries ~> "vertices" ~> 0 ~> "appBytes" === Some(S("103")))
    //assert(entries ~> "vertices" ~> 0 ~> "srcBytes" === Some(S("104")))
    //assert(entries ~> "vertices" ~> 0 ~> "dstBytes" === Some(S("105")))
    //assert(entries ~> "vertices" ~> 0 ~> "bytes" === Some(S("106")))
    //assert(entries ~> "vertices" ~> 0 ~> "srcPackets" === Some(S("107")))
    //assert(entries ~> "vertices" ~> 0 ~> "dstPackets" === Some(S("108")))
    //assert(entries ~> "vertices" ~> 0 ~> "packets" === Some(S("109")))
