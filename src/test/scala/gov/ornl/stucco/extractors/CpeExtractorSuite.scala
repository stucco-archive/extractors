...
class CpeExtractorSuite extends FunSuite {
  ...
  test("parse one simple CPE element") {
    ...
    assert(cpe ~> "vertices" ~> 0 ~> "modifiedDate" === Some(N(1292355512197L)))
  ...
  test("parse several CPE elements") {
    ...
    assert(cpe ~> "vertices" ~> 0 ~> "status" === Some(S("DRAFT")))
    assert(cpe ~> "vertices" ~> 0 ~> "modifiedDate" === Some(N(1189791409090L)))
    assert(cpe ~> "vertices" ~> 0 ~> "part" === Some(S("/a")))
    ...
    assert(cpe ~> "vertices" ~> 1 ~> "modifiedDate" === Some(N(1303309358607L)))
    ...
    assert(cpe ~> "vertices" ~> 2 ~> "modifiedDate" === Some(N(1303265333277L)))
